        ...
        for (size_t meshIx = 0; meshIx < model.GetMeshCount(); meshIx++) {
            ...
            if (materialIt == materialIds.end()) {
                ...
                for (const MaterialParameterDesc& parameter : frameMaterial.compiledTemplate->parameters) {
                    const MaterialParameterOverride* overrideValue = FindParameterOverride(*sceneMaterial, parameter.name);
                ...
                for (const MaterialSamplerDesc& sampler : frameMaterial.compiledTemplate->samplers) {
                    frameMaterial.textureIds.push_back(getTextureId(FindTextureBinding(*sceneMaterial, sampler.name)));
                }
...
void RenderSystem::PresentCompletedSoftwareFrame() {
    ...
    if (!completedFrame || completedFrame->pixels.empty() || completedFrame->width == 0 || completedFrame->height == 0) {
        UpdateSoftwareMemoryStats();
        return;
    }
...
void RenderSystem::UpdateSoftwareMemoryStats() {
    ...
    p_Stats_->softwarePresentedFrameBytes = m_PresentedSoftwareFrame ? m_PresentedSoftwareFrame->EstimateResidentMemory() : 0;
}
